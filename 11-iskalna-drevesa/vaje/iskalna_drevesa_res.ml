...
let leaf n : 'a bin_tree =
    Node (Empty, n, Empty)

let testni_primer = Node (Node (leaf 0, 2, Empty), 5, Node (leaf 6, 7, leaf 11))
...
let mirror (tree : 'a bin_tree) : 'a bin_tree =
    match tree with
    | Empty -> Empty
    | Node (t1, r, t2) -> Node (t2, r, t1)
...
let rec height (tree : 'a bin_tree) : int =
    match tree with
    | Empty -> 0
    | Node (t1, r, t2) -> 1 + max (height t1) (height t2)

let rec size (tree : 'a bin_tree) : int = 
    match tree with
    | Empty -> 0
    | Node (t1, r, t2) -> 1 + size t1 + size t2
...
let rec map_tree f (tree : 'a bin_tree) : 'a bin_tree =
    match tree with
    | Empty -> Empty
    | Node (t1, r, t2) -> Node (map_tree f t1, f r, map_tree f t2)
...
let rec list_of_tree (tree : 'a bin_tree) =
    match tree with
    | Empty -> []
    | Node (t1, r, t2) -> (list_of_tree t1) @ [r] @ (list_of_tree t2)
